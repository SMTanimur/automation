...
import { usePropId } from "@/hooks/usePropId";
...
const Progress: ProgressComponent = forwardRef<HTMLDivElement, ProgressProps>(
  (props: ProgressProps, ref?: ForwardedRef<HTMLDivElement>) => {
    ...
    useEffect(() => {
      ...
      progressBarRef.current.style.backgroundSize = progressStyles.stripe({
        size,
      });
    ...
    return (
      <div
        id={id}
        ref={ref}
        className={progressStyles.wrapper({ className, size })}
        {...additionalProps}
      >
        ...
        {description && (
          <span className={progressStyles.description({ size })}>
            {description}
          </span>
        )}
      </div>
